$(function(){ 
	...
	var addVideoFileInputEvent = function(){
		$('#video').find('input[type=file]:last').bind('change', function(){
			...
			currentID = currentID.substring(currentID.length-1, currentID.length);
			nextID = Number(currentID)+1;
			that.parent().parent().append('<div class="row"><label for="Video_video">Video</label><br /><input id="ytVideo_video_'+nextID+'" type="hidden" value="" name="Video[video]['+nextID+']" /><input size="50" maxlength="50" name="Video[video]['+nextID+']" id="Video_video_'+nextID+'" type="file" /></div>');
			that.unbind('change');
